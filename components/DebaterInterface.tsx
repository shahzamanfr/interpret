...
import {
  getDebateResponseWithGroq,
  getEnhancedDebateEvaluationWithGroq
...
interface DebaterInterfaceProps {
  ...
  grokApiKey?: string | string[]; // Groq API key(s) for debate
}
...
const DebaterInterface: React.FC<DebaterInterfaceProps> = ({ onBack, ai, grokApiKey }) => {
  ...
  const handleTopicSubmit = async () => {
    ...
    try {
      console.log("🤖 Using Grok for debate...");
      // Start the debate with AI opponent taking opposite stance
      const response = await getDebateResponseWithGroq(grokApiKey, debateTopic, "", 0, true);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await getDebateResponseWithGroq(
        grokApiKey,
        debateTopic,
        currentMessage,
        discussionRound,
  ...
  const handleEndDebate = async () => {
    ...
    try {
      ...
      const discussionHistory = messages.map((msg) => ({
        type: msg.type,
        content: msg.content,
        turnNumber: 0, // Add turnNumber for compatibility
      }));

      console.log("🤖 Using Grok for enhanced debate evaluation...");
      // Get comprehensive final evaluation using Grok
      const evaluation = await getEnhancedDebateEvaluationWithGroq(
        grokApiKey,
        debateTopic,
        discussionHistory,
        userStance,
  ...
  const handleTryAgain = () => {
    ...
    // Restart the debate with the same topic
    handleTopicSubmit();
  };
                ...
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                  <div className="flex-1">
                    <textarea
                      ...
                      className="w-full h-24 p-3 sm:p-4 bg-gray-800 border-2 border-gray-600 rounded-none text-white placeholder-gray-400 focus:outline-none focus:border-white transition-all duration-200 text-sm sm:text-base resize-none"
                      disabled={isLoading}
                      onKeyDown={(e) => {
